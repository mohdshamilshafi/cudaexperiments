...
Graph Colouring problem entails \textit{colouring/labeling} of the vertices/edges of a graph based on some set of conditions which are to be satisfied. In other words, it's a problem in which you allocate a colour/number to every vertex/edge of a graph such that a set of constraints are satisfied. There are different versions of Graph Colouring and the one which is of interest to us is that of Vertex Colouring.
...
A graph \textit{G} is said to \textit{k-}colourable, if \textit{G} can be coloured using \textit{k} colours. For example, from the \textit{Four Colour Theorem} (\citet{appel1977}; \citet{thomas98};\citet{wilson14}), we have that all planar graphs are 4-colourable. Also, all bipartite graphs are 2-colourable.
...
\State $\textit{conflictSet} \gets \phi$
\For{$each \: v \in V$} \Comment{Done in parallel}
\For{$each \: u \in V$ such that $u$ and $v$ are adjacent}
\If{$colour[v] = colour[u]$}
\State $\textit{conflictSet} \gets \textit{conflictSet} \cup min(v,u)$
\EndIf
\EndFor
\EndFor 
\BState \emph{Phase 3 (Conflict Resolution)}:
\For {$each \: v \in \textit{conflictSet}$}
\State $colour[v] \gets \textit{A colour by some sequential colouring algorithm}$
\EndFor
\EndProcedure
\end{algorithmic}
...
\State $\textit{Initialization}$
\BState \emph{GPU}:
\State $\textit{Initialize MaxDegree}$ \Comment{Found in parallel on GPU}
\For {$each \: v \in V$}\Comment{Done in parallel by |V| threads}
\State $colour[v] \gets CURAND(MaxDegree+1)+1$\Comment{RAND on GPU}
\EndFor
\State $\textit{conflictSet} \gets \phi$
\For{$each \: v \in V$} \Comment{Done in parallel}
\For{$each \: u \in V$ such that $u$ and $v$ are adjacent}
...
\BState \emph{CPU}:
\For {$each \: v \in \textit{conflictSet}$}
\State $colour[v] \gets \textit{A colour by some greedy colouring algorithm}$
\EndFor
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\For {$each \: v \in MaxS$}\Comment{Done in parallel}
\State $colour[v] \gets currentColour$
\EndFor
\For {$each \: v \in MinS$}\Comment{Done in parallel}
...
\State \textit{Thread 0 represents u and thread 1 represent v}
...
\Else
\State $\textit{newColour} \gets \textit{Smallest possible colour > possibleColour[threadId] for i}$
\BState \emph{Label 2}:
\If {$ newColour < oldColour $}
\State $colour[i] \gets newColour$
\EndIf
\EndIf
\Else
\BState \emph{Label 3}:
...
\BState \emph{NewEdge}:
\State \textit{Take the new edge to added, $\bar{e}$, as the input}
\State $\bar{u}, \bar{v} \gets \textit{endpoints of }\bar{e}$
\For {$each e \in edgeSet$}
\State $\textit{u, v} \gets \textit{endpoints of e}$

\State $setU \gets V-adj(u)$\Comment adj(k) gives adjacent vertices of k
\State $setV \gets V-adj(v)$
...
\State $\textit{edgeSet} \gets \phi \cup \bar{e}$
...
Pessimistic, Semi-optimistic, Optimistic
