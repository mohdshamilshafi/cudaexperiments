...
\end{figure}
Moore's law, which observes that the number of transistors present in an integrated circuit approximately doubles every two years, still stands valid. Figure \ref{fig:moore's law} shows Moore's Law in action. Processors, and hence computers, have grown faster and faster over years. More and more transistors meant the processors could run faster, at a faster frequency. Processors with better and better clock speeds were introduced every year since the 1980s until around 2004 when instead of single core processors running at faster clocks or higher clock speeds, multi-core processors started rolling out.
...
\BState \emph{GPU (Partition Colouring)}: \label{7.5}
...
\EndIf 
\State $\textit{goto} \: \textit{GPU (Partition Colouring)}$ \label{7.16}
\BState \emph{CPU (Conflict Resolution)}:
\For {$each \: v \in \textit{conflictSet}$} \label{7.18}
\State $colour[v] \gets \textit{A colour by some sequential colouring algorithm}$ \label{7.19}
\EndFor
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{figure}[h]
    ...
    \caption{
       The Directed Graph from Table \ref{table:1} and its CSR
    }
    \label{fig:csrd}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{
        The Undirected Graph from Table \ref{table:2} and its CSR
    }
    \label{fig:csru}
\end{figure}
\begin{figure}[h]
    ...
    \caption{
        The Undirected Graph from Table \ref{table:2} and its UCSR
    }
    \label{fig:csrun}
\end{figure}
